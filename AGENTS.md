...
- Update `PORT_PRIORITY.md` in the same change and annotate that file's list entry as ported.
- `PORT_PRIORITY.md` is the single canonical progress list for all Rust files in `ink-rust/`. Keep it in sync whenever a Rust file's status changes.
