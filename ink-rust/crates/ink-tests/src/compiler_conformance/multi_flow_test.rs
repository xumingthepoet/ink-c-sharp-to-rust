...
fn basics_test() -> Result<(), StoryError> {
    let json_string = common::get_ink_string("inkfiles/runtime/multiflow-basics.ink");
...
fn multiflow_save_load_threads() -> Result<(), StoryError> {
    let json_string = common::get_ink_string("inkfiles/runtime/multiflow-saveloadthreads.ink");
    let mut story = Story::new(&json_string);
