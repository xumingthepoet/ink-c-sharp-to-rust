...
36. `ParsedHierarchy/Weave.cs` (`deps=16`) [partial: reason=the weave helper now owns indentation-based hierarchy construction and weavepoint naming, but the full loose-end routing and termination analysis are still simplified; missing=exact loose-end propagation and full termination validation; next=finish the remaining flow-tree ownership model and nested sequence/conditional weaving]
