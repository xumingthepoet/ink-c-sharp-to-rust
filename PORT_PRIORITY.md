...
36. `ParsedHierarchy/Weave.cs` (`deps=16`) [partial: reason=the weave helper now owns the root-level runtime container and weavepoint naming, but the full nested weave hierarchy, loose-end routing, and termination analysis are still simplified; missing=exact recursive weave reconstruction, loose-end propagation, and full termination validation; next=finish the remaining flow-tree ownership model and nested sequence/conditional weaving]
